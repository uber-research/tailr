...
class SharedMixedModule(snt.AbstractModule):
    """Convolutional decoder."""
    ...
    def __init__(self,
                 ...
                 n_lin_layers=2,
                 name='shared_conv_encoder'):
        super(SharedMixedModule, self).__init__(name=name)

        self._filters = filters
        self._kernel_size = kernel_size
        self._activation = activation
        self.strides = strides
        #assert len(strides) == len(filters) - n_lin_layers
        self.conv_shapes = None
    ...
    def _build(self, x, is_training=True):
        ...
        for i, (filter_i,
            ...
            self.conv_shapes.append(h.shape.as_list())
        h = snt.BatchFlatten()(h)
        
        for i, layer_out_i in enumerate(self._filters[-self.n_lin_layers:]):
            enc_mlp = snt.nets.MLP(
                name='enc_mlp',
                output_sizes=[layer_out_i],
                activation=self._activation,
                activate_final=True)
            h = enc_mlp(h)
