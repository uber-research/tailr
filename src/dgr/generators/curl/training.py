...
# Dataset processing, this need a complete overhaul for translating to pytorch, will see with nicola
...
# TODO see how to translate the datasets' loading
...
def get_data_sources(dataset, dataset_kwargs, batch_size, test_batch_size,
    ...
    """
    # TODO Since torch uses a different function for each dataset, we could add a function that takes a string and returns the dataset ?
    ...
    if training_data_type == 'sequential':
        ...
        if n_concurrent_classes == 1:
            def filter_fn(v): return torch.equal(v[label_key], c)
        else:
            ...
            def filter_fn(v): return torch.logical_and(
                torch.ge(v[label_key], cmin[c]), torch.lt(
                    v[label_key], cmax[c]))
...
# This function is supposed to replace the one below
def get_eval_results(x, label, n_y, curl_model, classify_with_samples, is_training, name):
    """Gets Likelhood and KL divergence measures from the model.
    ...
    Returns:
      Measures of ELBO, LL, KL divergence etc...
    """
    ...
    # TODO find how to compute confusion matrix in torch
    confusion = torch.tensor(confusion_matrix(
        label, torch.argmax(cat_probs, dim=1)))
    purity = (torch.sum(torch.max(confusion, dim=0))
              / torch.sum(confusion))
    ...
    return elbo, ll, log_p_x, kl_y, kl_z, elbo_supervised, ll_supervised, log_p_x_supervised, kl_y_supervised, kl_z_supervised, cat_probs, confusion, purity, latents


# TODO Confusion Matrix and check whether the output should be kept that way
# Note that this function essentailly builds a graph, and pass it for execution much later
# Maybe should we move it somewhere else ...?
def setup_training_and_eval_graphs(x, label, y, n_y, curl_model,
    ...
    # TODO find how to compute confusion matrix in torch
    confusion = tf.confusion_matrix(label, torch.argmax(cat_probs, dim=1),
...
# TODO check that no changes are needed in this function
...
def setup_dynamic_ops(n_y):
    ...
    """
    ### TODO NOT SURE YET HOW TO TRANSLATE THIS ###
    ###############################################
    ...
    ind_from = tfc.placeholder(dtype=torch.int32)
    ind_to = tfc.placeholder(dtype=torch.int32)
    ###############################################
    ...
    w_indices_pre_permutation = torch.stack([
        torch.arange(start=0, end=cluster_w.shape[0], dtype=torch.int32),
        ind_to * torch.ones(cluster_w.shape[0], dtype=torch.int32)
    ])
    w_indices = w_indices_pre_permutation.permute(
        *[i for i in range(w_indices_pre_permutation.dim()-1, -1, -1)])

    b_indices = ind_to
    # Determine updates themselves
    cluster_w_updates = cluster_w.narrow(
        0, 0, cluster_w.shape[0]).narrow(1, ind_from, 1).squeeze()
    ...
    mu_indices_pre_permutation = torch.stack([
        ind_to * torch.ones(latent_prior_mu_w.shape[1], dtype=torch.int32),
        torch.arange(
            start=0, end=latent_prior_mu_w.shape[1], dtype=torch.int32)
    ])
    mu_indices = mu_indices_pre_permutation.permute(
        *[i for i in range(mu_indices_pre_permutation.dim() - 1, -1, -1)])

    mu_updates = latent_prior_mu_w.narrow(0, ind_from, 1).narrow(
        1, 0, latent_prior_mu_w.shape[1]).squeeze()
    mu_update_op = tf.scatter_nd_update(
        latent_prior_mu_w, mu_indices, mu_updates)

    sigma_indices_pre_permutation = torch.stack([
        ind_to *
        torch.ones(shape=(latent_prior_sigma_w.shape[1],), dtype=torch.int32),
        torch.arange(
            start=0, end=latent_prior_sigma_w.shape[1], dtype=torch.int32)
    ])
    sigma_indices = sigma_indices_pre_permutation.permute(
        *[i for i in range(sigma_indices_pre_permutation.dim() - 1, -1, -1)])

    sigma_updates = latent_prior_sigma_w.narrow(0, ind_from, 1).narrow(
        1, 0, latent_prior_sigma_w.shape[1]).squeeze()
...
# TODO again a very TF like function that will need a complete overhaul
...
# TODO big chunky function with parts that will be moved to the model.py file in the forward function of curl. Do not break it down yet (for debugging purpose and comparison with the OG code)
...
def run_training(
    ...
    n_y_active = nn.Parameter(
        data=torch.tensor([n_y_active_np], dtype=torch.int32),
        requires_grad=False)
    ...
    # TODO "I hate translating tf placeholders to torch" - Some poor stupid intern
    # This would most likely move in the training loop ?
    x_train_raw = tfc.placeholder(
        dtype=torch.float32, shape=(batch_size,) + output_shape)
    label_train = tfc.placeholder(dtype=torch.int32, shape=(batch_size,))
    ...
    def binarize_fn(x):
        ...
        """
        return torch.gt(x, 0.5 * torch.ones_like(x)).double()
    ...
    prior_train = torch.distributions.one_hot_categorical.OneHotCategorical(
        probs=prior_train_probs)

    prior_test_probs = utils.construct_prior_probs(
        test_batch_size, n_y, n_y_active)
    prior_test = torch.distributions.one_hot_categorical.OneHotCategorical(
        probs=prior_test_probs)
    ...
    ### GRAPH SETUP ###
    # TODO how do I translate this torch? This would be directly moved to the training loop probably
    ...
    if classify_with_samples:
        latents_for_clf = model_eval.infer_latent(
            hiddens=hiddens_for_clf, y=cat_for_clf.sample()).double().sample()
    else:
        latents_for_clf = model_eval.infer_latent(
            hiddens=hiddens_for_clf, y=cat_for_clf.mode()).double().mean()
    ...
    # TODO This whole thing should be instantiated outstide training loop then updated in the loop. so not sure where to move them for now
    global_step = 0
    lr_schedule = [
        (el * num_train_examples / batch_size).type(torch.int64)
        for el in lr_schedule
    ]
    num_schedule_steps = torch.sum(
        global_step >= lr_schedule).type(torch.float32)
    lr = float(lr_init) * float(lr_factor)**num_schedule_steps

    # TODO properly define the optimizer (no this is not how it supposed to be define, you need to give the parameters to be optimized)
    # TODO make the model_train.model be an actual net ?
    optimizer = torch.optim.Adam(model_train.model.parameters(), lr=lr)
    # TODO I have no clue how to properly translate the control dependencies line
    # The minimizing steps will be done in the training loop
    #
    ...
    if gen_every_n > 0:
        ...
        y_gen = torch.distributions.one_hot_categorical.OneHotCategorical(
    ...
    # TODO This is the large training loop that you should carefully modify
