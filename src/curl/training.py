...
result_root = f'/results/curl/{time_str}'
tb_logdir = f'{result_root}/tb_logs'
csv_logdir = f'{result_root}/csv_log'
img_logdir = f'{result_root}/img'
...
def run_training(
  ...
  Path(img_logdir).mkdir(parents=True, exist_ok=True)
  Path(csv_logdir).mkdir(parents=True, exist_ok=True)
  Path(cluster_weight_logdir).mkdir(parents=True, exist_ok=True)
  ...
  exp_buffer_size = batch_size  # Size of the buffer of poorly explained data
  ...
  def binarize_fn(x):
    ...
    exp_wait_steps = 100  # Steps to wait after expansion before eligible again
    exp_burn_in = 100  # Steps to wait at start of learning before eligible
    exp_buffer_size = max(100, batch_size)  # Size of the buffer of poorly explained data
    num_buffer_train_steps = 20  # Num steps to train component on buffer
    ...
    if gen_every_n > 0:
        ...
        knn_models = []
        gen_latents = model_train.generate_latent(y=y_gen).mean()
    ...
    cumulative_component_weights = np.array([0] * n_y).astype(float)
    recent_component_weights = np.array([0] * n_y).astype(float)
    ...
    for step in range(n_steps):
      ...
      if (gen_refresh_period and step % gen_refresh_period == 0 and
        ...
        cumulative_component_weights += recent_component_weights
        recent_component_weights = np.zeros(n_y)

        # Generate enough samples for the rest of the next period
        # (Functionally equivalent to storing and sampling from the model).
        gen_buffer_images, gen_buffer_labels, _ = training_utils.get_generated_data(
            sess=sess,
            gen_op=gen_samples,
            y_input=y_gen,
            gen_buffer_size=gen_buffer_size,
            component_counts=cumulative_component_weights)
    ...
    if train_supervised:
        ...
        loss_summary = tf.summary.scalar(
            name='OG_CURL_train_loss', tensor=tf.reshape(train_ops.elbo, []))
        merged_summary = tf.summary.merge_all()

        oracle, _ = classifier_utils.init_classifier_and_optim(
            'dgr_clf', n_classes)
        oracle.load_state_dict(torch.load('./src/curl/oracle_classifier.pth'))
...
<<<<<<< HEAD
          ...
          if n_poor_data >= exp_buffer_size:
            ...
            if gen_refresh_on_expansion and gen_every_n > 0:
              ...
              gen_buffer_images, gen_buffer_labels, _ = training_utils.get_generated_data(
                  ...
                  component_counts=cumulative_component_weights,
                  labeling_clf=oracle,
                  log_file_name=f'{csv_logdir}/cluster_class_count_{n_y_active_np}',
                  )
              save_path = f'{img_logdir}'
      ...
      if used_real_data:
        ...
        recent_component_weights += np.sum(
  ...
  with open(f'/results/DONE', 'w+') as done_file:
    done_file.write('-What is my purpose? -You terminate the run. -Oh my god...')
=======
        file_writer = tf.summary.FileWriter(
            f'{tb_logdir}', graph=sess.graph)
        ...
        for step in range(n_steps):
            ...
            ### 1) PERIODICALLY TAKE SNAPSHOTS FOR GENERATIVE REPLAY ###
            if (gen_refresh_period and step + 1 % gen_refresh_period == 0 and
                    gen_every_n > 0):
...
>>>>>>> 2b3c34bffb312c69d3e9ffc2efbe91b0c7cc8da5
